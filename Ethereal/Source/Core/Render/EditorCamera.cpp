...
namespace Ethereal
{
	...
	{
		if (Input::IsKeyPressed(Key::LeftCtrl))
		{
			const glm::vec2& mouse{ Input::GetMouseX(), Input::GetMouseY() };
			glm::vec2 delta = (mouse - m_InitialMousePosition) * 0.003f;
			m_InitialMousePosition = mouse;

			if (Input::IsMouseButtonPressed(Mouse::MiddleClick))
				MousePan(delta);
			else if (Input::IsMouseButtonPressed(Mouse::LeftClick))
				MouseRotate(delta);
			else if (Input::IsMouseButtonPressed(Mouse::RightClick))
				MouseZoom(delta.y);
